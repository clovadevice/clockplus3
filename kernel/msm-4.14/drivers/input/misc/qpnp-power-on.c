...
#if defined(CONFIG_PRODUCT_IF_S1300N) || defined(CONFIG_PRODUCT_IF_MKT300) || defined(CONFIG_PRODUCT_IF_S1500N) || defined(CONFIG_PRODUCT_IF_S1600N)
#define OVLO_SETTING
#endif

#ifdef OVLO_SETTING
#define QPNP_PON_OVLO(pon)			((pon)->base + 0x86)
// OVLO setting 5.7V
#define OVLO_VOLTAGE				0x8F
#endif
...
#ifdef OVLO_SETTING
	qpnp_pon_masked_write_backup(pon, QPNP_PON_SEC_ACCESS(pon),
					0xFF, QPNP_PON_SEC_UNLOCK);
	rc = regmap_write(pon->regmap, QPNP_PON_OVLO(pon), OVLO_VOLTAGE);
	if (rc) {
		dev_err(pon->dev, "Register write failed, addr=0x%04X, rc=%d\n", QPNP_PON_OVLO(pon), rc);
	}
#endif
