...
#define PERIOD_TO_HZ(period_ns) ((1  * (uint32_t)1000000000) / period_ns)
...
static int pdm_pwm_calc_pwm_frequency(struct pdm_pwm_chip *chip,
					int period_ns, u32 hw_idx)
{
	...
	if (!freq) {
		pr_err("%s: Frequency cannot be Zero\n", __func__);
		return -EINVAL;
	}	

	if (freq > (chip->pwm_core_rate >> 1) || freq <
						(chip->pwm_core_rate >> 16)) {
		pr_info("%s: Freq %ld is not in range Max=%ld Min=%ld\n", __func__, freq,
		(chip->pwm_core_rate >> 1), (chip->pwm_core_rate >> 16));
		return -ERANGE;
	}
...
{
	unsigned long calc_period, calc_duty;
	uint16_t cyc_offset1=0, cyc_offset2=0;
	 ...
	 * 19.2MHz.
	 * 3. max_freq = pwm_core_rate/2 ^ 16;
	 * 4. min_freq = pwm_core_rate/2;
	 * 5. calculate the frequency based on the period_ns and compare.
	 */
	ret = clk_prepare_enable(chip->pdm_ahb_clk);
	if (ret) {
		pr_err("%s: fail to enable pdm_ahb_clk\n", __func__);
		return ret;
	}

	ret = clk_prepare_enable(chip->pwm_core_clk);
	if (ret) {
		pr_err("%s: fail to enable pwm_core_clk\n", __func__);
		goto fail;
	}

	mutex_lock(&chip->frames[hw_idx].frame_lock);

	ret = pdm_pwm_calc_pwm_frequency(chip, current_period, hw_idx);
	if (ret) {
		pr_err("%s: fail to calc pwm freq\n", __func__);
		goto out;
	}
	...
	calc_period = DIV_ROUND_CLOSEST(chip->pwm_core_rate,
					chip->frames[hw_idx].current_freq);
	pr_debug("%s: pwm_core_rate/current_freq = %lu\n", __func__, calc_period);
	
	calc_duty = DIV_ROUND_CLOSEST(calc_period * (DIV_ROUND_CLOSEST((duty_ns * 100),
							current_period)), 100);
	pr_debug("%s: duty_ns(%lu)/current_period(%lu) = %lu\n", __func__, duty_ns, current_period, calc_duty);
	
	
	cyc_offset1 = ((calc_duty) >> 0 ) & 0xffff;
	cyc_offset2 = ((calc_duty) >> 16) & 0xffff;
 
	ret = regmap_update_bits(chip->regmap, chip->frames[hw_idx].reg_offset
					+ PWM_CTL1, GENMASK(15, 0), cyc_offset1);
	if (ret)
		goto out;
	
	ret = regmap_update_bits(chip->regmap, chip->frames[hw_idx].reg_offset
					+ PWM_CTL2, GENMASK(15, 0), cyc_offset2);
	if (ret)
		goto out;
...
static int pdm_pwm_enable(struct pdm_pwm_chip *chip, struct pwm_device *pwm)
{
	...
	ret = clk_prepare_enable(chip->pdm_ahb_clk);
	if (ret) {
		pr_err("%s: fail to ahb_clk prepare\n", __func__);
		return ret;
	}
	...
	if (ret) {
		clk_disable_unprepare(chip->pdm_ahb_clk);
		pr_err("%s: fail to core_clk prepare\n", __func__);
		return ret;
	}
					...
					struct pwm_state *state)
{
	...
	if (state->period < curr_state.period) {
		pr_err("%s: Error set period: %lu, curr period: %lu\n", __func__, 
					state->period, curr_state.period);
		return -EINVAL;
	}
...
static void pdm_pwm_dbg_show(struct pwm_chip *pwm_chip, struct seq_file *s)
{
	...
	for (i = 0; i < pwm_chip->npwm; i++) {
		...
		seq_printf(s, "current duty       (%lu)  =  %lu\n\n",
...
static int pdm_pwm_pinctrl_select_state(struct pdm_pwm_chip *chip, bool enable)
{	
	int ret = 0;
	
	if(enable) {
		ret = pinctrl_select_state(chip->pinctrl, chip->pinctrl_active);
		pr_debug("%s: %s(%d)\n", __func__, enable?"active":"sleep", ret);
	}
	else {
		ret = pinctrl_select_state(chip->pinctrl, chip->pinctrl_sleep);
		pr_debug("%s: %s(%d)\n", __func__, enable?"active":"sleep", ret);
	}
				...
				struct pdm_pwm_chip *chip)
{
	...
	if (IS_ERR(chip->pdm_ahb_clk)) {
		if (PTR_ERR(chip->pdm_ahb_clk) != -EPROBE_DEFER)
			pr_err("%s: Unable to get ahb clock handle\n", __func__);
	...
	if (IS_ERR(chip->pwm_core_clk)) {
		if (PTR_ERR(chip->pwm_core_clk) != -EPROBE_DEFER)
			pr_err("%s: Unable to get core clock handle\n", __func__);
	...
	if (res == NULL) {
		pr_err("%s: Failed to get reg base resource\n", __func__);
		return -EINVAL;
	}
	...
	if(IS_ERR_OR_NULL(chip->pinctrl)) {
		pr_err("%s: Cannot get gpio pinctrl: %ld\n", __func__, 
					PTR_ERR(chip->pinctrl));
		goto err_pinctrl;		
	} 		
	
	chip->pinctrl_active = pinctrl_lookup_state(chip->pinctrl, "pwm_led_act");
	if(IS_ERR_OR_NULL(chip->pinctrl_active)) {
		pr_err("%s: Cannot get pwm_led_act pinctrl state: %ld\n", __func__,
			PTR_ERR(chip->pinctrl_active));
		goto err_pinctrl;
	}
			
	chip->pinctrl_sleep = pinctrl_lookup_state(chip->pinctrl, "pwm_led_sus");
	if(IS_ERR_OR_NULL(chip->pinctrl_sleep)) {
		pr_err("%s: Cannot get pwm_led_sus pinctrl state: %ld\n", __func__,
			PTR_ERR(chip->pinctrl_sleep));
		goto err_pinctrl;
	}
	
	pdm_pwm_pinctrl_select_state(chip, true);
...
static int pdm_pwm_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_err("%s: Devicetree properties parsing failed, rc=%d\n", __func__,
				rc);
		goto err_out;
	}
	...
	pr_info("%s: pwmchip driver success.\n", __func__);
	return 0;
err_out:
...
static int pdm_pwm_remove(struct platform_device *pdev)
{
	...
	if (rc < 0)
		pr_err("%s: Remove pwmchip failed, rc=%d\n", __func__, rc);
